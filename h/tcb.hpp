...
public:
    ...
    void setTimeSlice(uint64 timeSlice) { this->timeSlice = timeSlice; }
    // static int time_sleep(time_t time);

    uint64 getTimeSlice() const { return timeSlice; }

    bool isSleeping() const {
        return timeSleepCounter > 0;
    }
    ...
    static int time_sleep(time_t time);
    // Define error codes as static const members
    static const uint64 SLEEP_SUCCESS = 0;
